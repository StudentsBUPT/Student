...
public class ClassBookJdbcImpl implements ClassBook {
	...
	public List selectAllclassStuDrop(String studentid) {
		...
		JdbcUtils jdbc =new JdbcUtils();
				try {
					conn = jdbc.getConection();
					String sql = "select DISTINCT b.classid, a.teachername, b.id, b.classname, b.classmust,b.classs, c.bookhouse, e.chengji from student_teacher as a,student_class as b, teacher_book as c, student_book as e where b.classname = e.bookname and a.teacherid=c.teacherid and b.classname=c.bookname and e.studentid = ?";
					String  sql2="select * from student_class";
					st= conn.prepareStatement(sql);
					st.setString(1, studentid);
					rs= st.executeQuery();
					...
					while(rs.next()){
						...
						if(rs.getString("chengji").equals("")){
